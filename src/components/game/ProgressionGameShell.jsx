    ...
    <motion.div className="fixed inset-0 z-[80] flex items-center justify-center p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <motion.div initial={{ y: 24, scale: .96 }} animate={{ y: 0, scale: 1 }} className="relative max-w-lg w-full rounded-3xl border border-fuchsia-400/30 bg-[#101018] p-6 shadow-2xl">
        <div className="text-center mb-5">
          ...
          <p className="text-xs text-muted-foreground mt-2">This trait lasts for the rest of the current run and is recorded if you survive.</p>
        </div>
          ...
          {offers.map(trait => (
            <button key={trait.id} onClick={() => onChoose(trait)} className="text-left rounded-2xl border border-white/10 bg-white/[.035] p-4 hover:border-cyan-300/50 hover:bg-cyan-300/[.06] transition">
              <div className="flex items-center justify-between gap-3">
                <strong className="text-base">{trait.name}</strong>
                <span className="text-[9px] uppercase tracking-widest text-cyan-300">{trait.rarity}</span>
              </div>
  ...
  return (
    <motion.div initial={{ x: 30, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 30, opacity: 0 }} className="fixed right-3 top-20 z-50 w-[min(330px,calc(100vw-24px))] rounded-2xl border border-cyan-300/25 bg-[#0e0e16]/95 p-4 shadow-2xl backdrop-blur-xl">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2"><Target className="w-4 h-4 text-cyan-300" /><span className="text-xs uppercase tracking-widest font-bold">Run objectives</span></div>
        ...
        {Object.values(objectives).map(objective => (
          <div key={objective.id} className="rounded-xl border border-white/10 p-3 bg-white/[.025]">
            <div className="flex justify-between gap-2">
              ...
              <span className={objective.completed ? "text-emerald-300 text-xs" : "text-muted-foreground text-xs"}>{objective.completed ? "Complete" : `${objective.progress}/${objective.target}`}</span>
            </div>
...
export default function ProgressionGameShell({ children }) {
  const { player, party, choiceCount, temporaryTraits, addTemporaryTrait, discoveredClues, objectiveState, updateObjective, activeModifiers } = useGame();
  ...
  const chooseTrait = trait => {
    addTemporaryTrait(trait);
    setOffers([]);
  };
  ...
  return (
    <>
      {children}
      {player && (
        <button onClick={() => setShowObjectives(v => !v)} className="fixed right-3 bottom-4 z-40 rounded-full border border-cyan-300/30 bg-[#101018]/95 px-4 py-2 text-xs font-bold shadow-xl backdrop-blur-xl">
          <span className="text-cyan-300">Objectives</span>
          {activeModifiers?.length > 0 && <span className="ml-2 text-fuchsia-300">Challenge</span>}
        </button>
      )}
      ...
      <AnimatePresence>{offers.length > 0 && <TraitChoiceModal offers={offers} onChoose={chooseTrait} />}</AnimatePresence>
    </>
  );
}
